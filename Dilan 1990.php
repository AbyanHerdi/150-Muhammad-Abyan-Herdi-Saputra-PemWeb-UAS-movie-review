<?php
// File: dilan 1990.php (FIXED - Show All Reviews + Watchlist + Trailer Button)
...
$defaultTitle = 'Dilan 1990';
$defaultPoster = 'uploads/posters/1763311429.png';
$defaultYear = '2018';
$defaultGenre = 'Drama, Romantis';
$defaultDuration = '1h 50m';
$defaultDirector = 'Fajar Bustomi';
$defaultCast = 'Iqbaal Ramadhan, Vanesha Prescilla';
...
$duration = $movie['duration'] ?? $defaultDuration;
$director = $movie['director'] ?? $defaultDirector;
$cast = $movie['cast'] ?? $defaultCast;

// 🔥 FIXED: Render HTML untuk SEMUA ulasan (bukan hanya user saat ini)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['is_ajax']) && $_POST['is_ajax'] == 1) {
    ...
    try {
        ...
        $executeResult = $stmt->execute([$MOVIE_ID, $USER_ID, $rating, $reviewText]);
        
        if (!$executeResult) {
            throw new Exception("❌ ERROR Execute Failed!");
        }
...
// 🔥 FIXED: Ambil SEMUA ulasan (sama seperti ulasan_dilan1990.php)
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        textarea { width: 100%; min-height: 120px; padding: 10px; border-radius: 4px; border: 1px solid #444; background-color: var(--input-bg); color: var(--text-light); resize: vertical; }
        .submit-btn { background-color: var(--primary-color); color: var(--text-light); padding: 10px 25px; border: none; border-radius: 5px; font-weight: 600; cursor: pointer; transition: background-color 0.3s; }
        ...
        .star-rating label:hover, .star-rating label:hover ~ label, .star-rating input:checked ~ label { color: var(--star-gold); }
...
<body>
    <div class="container">
        ...
        <section class="movie-header">
            ...
            <div class="movie-info">
                ...
                <p><?= htmlspecialchars($releaseYear); ?> • <?= htmlspecialchars($genre); ?> • <?= htmlspecialchars($duration); ?></p>
                <p>Sutradara: <?= htmlspecialchars($director); ?> • Pemain: <?= htmlspecialchars($cast); ?></p>
                
                <div style="display: flex; gap: 10px; align-items: center; margin-top: 10px;">
                    <button id="likeButton" class="submit-btn like-btn" data-movie-id="DILAN_1990">
                    ...
                    <a href="ulasan_dilan1990.php" class="submit-btn review-link" style="text-decoration: none; display: inline-block;">
                ...
                <div class="rating-summary" style="margin-top: 25px;">
                    ...
                    <button id="trailerButton" class="submit-btn trailer-btn" style="padding: 15px 20px; background-color: #9b59b6;">
        ...
        <section class="review-form-section">
            ...
            <form id="reviewForm">
                ...
                <div class="form-group">
                    ...
                    <textarea id="reviewText" name="reviewText" placeholder="Tuliskan ulasan Anda tentang <?= htmlspecialchars($title); ?> di sini..."></textarea>
                </div>
    ...
    <script>
        ...
        // URL Trailer - Ganti dengan URL trailer film yang sesuai
        const TRAILER_URL = 'uploads/videos/Official Trailer Dilan 1990.mp4'; // Ganti dengan link trailer YouTube
        ...
        function loadReviewsAndStats() {
            ...
            submitButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Memuat...';
            
            fetch('dilan 1990.php?fetch_data=1')
